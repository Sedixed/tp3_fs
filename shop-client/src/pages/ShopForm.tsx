...
const ShopForm = () => {
    ...
    const validate = () => {
        ...
        return validateOpeningHours() && Object.values(schema(shop)).every((o) => o == '');
    };

    const validateOpeningHours = () => {
      for (let i = 0; i < shop.openingHours.length; ++i) {
        let opH = shop.openingHours[i]
        let opHOpeningDate = new Date(`1/1/1999 ${opH.openAt}`)
        let opHClosingDate = new Date(`1/1/1999 ${opH.closeAt}`)
        let otherOpH = shop.openingHours.filter((openingHour, id) => id !== i && opH.day === openingHour.day)
        for (let other of otherOpH) {
          const otherOpeningDate = new Date(`1/1/1999 ${other.openAt}`)
          const otherClosingDate = new Date(`1/1/1999 ${other.closeAt}`)
          if ((otherOpeningDate < opHOpeningDate && opHOpeningDate < otherClosingDate) ||
            (otherOpeningDate < opHClosingDate && opHClosingDate < otherClosingDate)
          ) {
            setToast({
              severity: 'error',
              message: 'Les horaires d\'ouverture se superposent !'
            })
            return false
          }
        }
